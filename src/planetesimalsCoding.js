...
end game is kinda unexciting... gravity doesn't feel relavent after too many asteroids are gone
  smaller map?
  goal not a complete clear, maybe just hit 10 asteroids
  goal is just survival and just spawn new asteroids to keep the total mass interesting
    you could start will a lower inital spawn if there is an ongoing spawn to produce a more ramping up feel.
    
maybe add the ability to put up shields to survive collisions?
maybe add durability regeneration
maybe spawn the asteroids closer together
mini map
in-game zoom?
fix new mass spawn so that it will pick a new location if the first spawn location already has a mass

//add in my own walls to speed up performance.

returns bodies in the array of bodies, mass, in that range
http://brm.io/matter-js/docs/classes/Query.html
Matter.Query.region(mass, {
  max:{
    x:200,
    y:200
  },
...
function planetesimals() {
    ...
    function levelScaling() {
      ...
      game.scale = 1 / (Math.log(game.level + 1)); //0.6 + 1.0 / (game.level); //controls map zoom
  ...
  //     World.add(engine.world, Bodies.circle(game.width * 0.5, game.height * 0.5, 300,{ 
  //         friction: 0,
  //         frictionStatic: 0,
  //         frictionAir: 0,
  //         restitution: 1,
